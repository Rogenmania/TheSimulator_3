...
for tii = 1:AgentNumber
    ...
    %CASManager?
    GCS(tii).SetInit(UVW_b(:,tii),VTP_g(:,tii),[XYZ_g(:,tii) XYZfin_g(:,tii)],NTurnRate,GoalvPath);
    CAS(tii).SetInit(UVW_b(:,tii),VTP_g(:,tii),[XYZ_g(:,tii) XYZfin_g(:,tii)],ATurnRate,ADist,VOpPo,VOpVee,DecMode);
