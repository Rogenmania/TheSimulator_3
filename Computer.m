    ...
    methods(Static)
        ...
        function RoMa = RotMat(AngleNya,Dimension)
            switch Dimension
                ...
                case -3 %true 3 dimenstional rotational matrix, but reversed - fdrom Stevens and Lewis
                    RolTi = -AngleNya(1); PitTi = -AngleNya(2); YawTi = -AngleNya(3);
                    RolMa = [1 0 0; 0 cos(RolTi) sin(RolTi); 0 -sin(RolTi) cos(RolTi)];
                    PitMa = [cos(PitTi) 0 -sin(PitTi); 0 1 0; sin(PitTi) 0 cos(PitTi)];
                    YawMa = [cos(YawTi) sin(YawTi) 0; -sin(YawTi) cos(YawTi) 0; 0 0 1];
                    RoMa = YawMa*PitMa*RolMa; %PTV
                otherwise
                    RoMa = [cos(AngleNya) -sin(AngleNya); sin(AngleNya) cos(AngleNya)];
            end
        end
